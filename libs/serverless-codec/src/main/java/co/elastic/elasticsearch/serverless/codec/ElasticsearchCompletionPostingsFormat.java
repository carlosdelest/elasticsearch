 ...
 * configuration.
 */
public abstract class ElasticsearchCompletionPostingsFormat extends CompletionPostingsFormat {
    ...
    static final Logger logger = LogManager.getLogger(ElasticsearchCompletionPostingsFormat.class);
    static final SetOnce<Boolean> fstOnHeap = new SetOnce<>();
    ...
    public static void configureFSTOnHeap(boolean fstOnHeap) {
        try {
            ElasticsearchCompletionPostingsFormat.fstOnHeap.set(fstOnHeap);
            ...
            logger.error(
                Strings.format(
                    ...
                    fstOnHeap,
                    ElasticsearchCompletionPostingsFormat.fstOnHeap.get()
                )
            );
        }
    }

    private final CompletionPostingsFormatOffHeap offHeapDelegate;
    private final String delegatePostingsFormatName;

    protected ElasticsearchCompletionPostingsFormat(String name, String delegatePostingsFormatName) {
        super(name, FSTLoadMode.ON_HEAP);
        this.offHeapDelegate = new CompletionPostingsFormatOffHeap(name, delegatePostingsFormatName);
        this.delegatePostingsFormatName = delegatePostingsFormatName;
    }

    @Override
    protected final PostingsFormat delegatePostingsFormat() {
        return PostingsFormat.forName(delegatePostingsFormatName);
    }

    @Override
    public FieldsProducer fieldsProducer(SegmentReadState state) throws IOException {
        if (Boolean.TRUE.equals(ElasticsearchCompletionPostingsFormat.fstOnHeap.get())) {
            ElasticsearchCompletionPostingsFormat.logger.trace("On Heap fieldsProducer called");
            return super.fieldsProducer(state);
        } else {
            ElasticsearchCompletionPostingsFormat.logger.trace("Off Heap fieldsProducer called");
    ...
    private static class CompletionPostingsFormatOffHeap extends CompletionPostingsFormat {
        ...
        private CompletionPostingsFormatOffHeap(String name, String delegatePostingsFormatName) {

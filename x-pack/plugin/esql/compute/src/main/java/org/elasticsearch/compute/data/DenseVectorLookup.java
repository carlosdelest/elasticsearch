 ...
 */
final class DenseVectorLookup implements ReleasableIterator<DenseVectorBlock> {
    private final DenseVectorBlock values;
    ...
    DenseVectorLookup(DenseVectorBlock values, IntBlock positions, ByteSizeValue targetBlockSize) {
        values.incRef();
        positions.incRef();
        this.values = values;
    ...
    @Override
    public DenseVectorBlock next() {
        try (DenseVectorBlock.Builder builder = positions.blockFactory().newDenseVectorBlockBuilder(positions.getTotalValueCount())) {
            int count = 0;
            while (position < positions.getPositionCount()) {
                ...
                switch (valuesInPosition) {
                    ...
                    case 1 -> builder.appendDenseVector(first);
    ...
    private void copy(DenseVectorBlock.Builder builder, int valuePosition) {
        ...
        for (int i = start; i < end; i++) {
            if (valuesInPosition == 0) {
                first = values.getDenseVector(i);
                valuesInPosition++;
                continue;
            }
            if (valuesInPosition == 1) {
                builder.beginPositionEntry();
                builder.appendDenseVector(first);
            }
            ...
            builder.appendDenseVector(values.getDenseVector(i));
            valuesInPosition++;
        }
    }

...
export X86_IMAGE_TAG=${DOCKER_IMAGE}:${BUILDKITE_COMMIT:0:7}-x86_64
export ARM_IMAGE_TAG=${DOCKER_IMAGE}:${BUILDKITE_COMMIT:0:7}-aarch64
...
docker manifest create ${DOCKER_IMAGE}:${BUILDKITE_COMMIT:0:7} --amend ${X86_IMAGE_TAG} --amend ${ARM_IMAGE_TAG}
docker manifest create ${DOCKER_IMAGE}:latest --amend ${X86_IMAGE_TAG} --amend ${ARM_IMAGE_TAG}
docker manifest push ${DOCKER_IMAGE}:${BUILDKITE_COMMIT:0:7}
